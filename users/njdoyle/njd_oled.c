...
static const char PROGMEM njd_sprite_layout[4*12] = {
	...
	0x02,0x3A, 0x02,0x2C, 0x06,0x0C, 0x34,0x38, 0x35,0x39, 0x06,0x0C,
	0x02,0x3E, 0x02,0x29, 0x03,0x09, 0x30,0x3C, 0x31,0x3D, 0x03,0x09,
	0x06,0x0E, 0x26,0x2C, 0x04,0x04, 0x04,0x04, 0x04,0x04, 0x02,0x3A,
	0x03,0x09, 0x03,0x09, 0x03,0x09, 0x03,0x09, 0x03,0x09, 0x02,0x3E
};
...
static const char PROGMEM njd_sprite_os[2*18] = {
	/* macOS                       Linux                          Windows */
	0x36,0x08,0x26,0x08,0x04,0x04, 0x06,0x08,0x69,0x68,0x37,0x3B, 0x02,0x3A,0x69,0x68,0x06,0x08,
	0x32,0x08,0x23,0x08,0x03,0x09, 0x03,0x08,0x61,0x60,0x33,0x3F, 0x02,0x3E,0x61,0x60,0x03,0x08
};
...
void oled_task_user(void) {
	if (is_master) {
		...
		for (int row = 0; row < 4; ++row) {
			// LAYOUT
			njd_oled_write_chars_P(njd_sprite_layout + (row*12) + (layout*2), 2);

			// OS
			if (0 == row || 3 == row)
				njd_oled_write_chars_P(njd_sprite_os_space, 6);
			else {
				njd_oled_write_chars_P(njd_sprite_os + ((row-1) * 18) + (6*os), 6);
			}

			// LAYER
			njd_oled_write_chars_P(njd_sprite_os_space, 4);
